<template>
    <div :class="typeaheadState" class="autocomplete">
        <div class="input-field" ref="toggle" @mousedown.prevent="toggle">
            ...
            >
            <label class="typeahead__text" for="search" ref="text">{{displayText.Worker_name}} {{displayText.Firm}}</label>
        </div>
        <ul class="typeahead__list" ref="list" v-if="open">
            <li class="typeahead__item" v-for="(option, index) in filteredOptions" :key="index">
                ...
                >
                    <span>{{option.Worker_name}}</span>
                    <span>|</span>
                    <span>{{option.Firm}}</span>
                </a>
            </li>
        </ul>
    </div>
    ...
    computed: {
      ...
      filteredOptions() {
        ...
        return this.options.filter((option) => {
          return ( exp.test(option.Id) || exp.test(option.Worker_name))
        })
      }
    },
    methods: {
