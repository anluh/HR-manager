  ...
  <div class="salary">
    ...
    <div class="page-wrapper">
      <transition name="slide-fade">
        <div v-if="hoursErr" class="toast toast--error">
          {{ $t("hours_added") }}
        </div>
      </transition>
      ...
      >
        <div
          class="add-salary__data"
          :class="{ disabled: !$v.month.isDate || !$v.month.required }"
        >
          <div class="input-field col s12 m6">
            ...
            <span
              class="error danger"
              v-show="$v.newHourRow.$dirty && !$v.newHourRow.Firm.required"
              >{{ $t("required") }}</span
            >
          </div>
          <div class="input-field">
            ...
            <span
              class="error danger"
              v-show="
                $v.newHourRow.$dirty && !$v.newHourRow.Worker.Name.required
              "
              >{{ $t("required") }}</span
            >
          </div>
          <div class="input-field">
            ...
            <span
              class="error danger"
              v-show="$v.newHourRow.$dirty && !$v.newHourRow.Hours.required"
              >{{ $t("required") }}</span
            >
            <span
              class="error danger"
              v-show="
                $v.newHourRow.$dirty &&
                $v.newHourRow.Hours.required &&
                !$v.newHourRow.Hours.isNumber
              "
              >{{ $t("number_valid") }}</span
            >
          </div>
          <button
            :disabled="!$v.month.isDate"
      ...
      <div class="hide-history">
        <a @click.prevent="hideHistory = !hideHistory" href="#">
          <span v-if="!hideHistory"
            ><i class="fas fa-eye-slash"></i>{{ $t("hide_history") }}</span
          >
          <span v-if="hideHistory"
            ><i class="fas fa-eye"></i>{{ $t("show_history") }}</span
          >
        </a>
      </div>
          ...
          <thead>
            <tr>
              ...
              <th class="table-total">
                <mark
                  ><div>{{ totalHours }} hr</div>
                  <div>{{ totalSalary | numberFormatter }} Kč</div></mark
                >
              </th>
            </tr>
          </thead>
              ...
              <td>
                <div>
                  ...
                  <modal @submit="deleteHistory(history)" submit-btn="Delete">
                    ...
                    ></i>
                    <div slot="popup-text">{{ $t("delete_hours") }}</div>
                  </modal>
                </div>
              </td>
            </tr>
...
export default {
  ...
  computed: {
    totalHours() {
      return this.histories
        .reduce((a, i) => (a = a + this.WithComaToFloat(i.Hours)), 0)
        .toFixed(1);
    },
    totalSalary() {
      return this.histories
        .reduce(
          (a, i) =>
            (a =
              a + this.WithComaToFloat(i.Hours) * this.WithComaToFloat(i.Rate)),
          0
        )
        .toFixed(0);
    },
  ...
  watch: {
    ...
    month() {
      ...
      this.newHourRow.MonthEnd = moment(this.month, "MM.YYYY")
        .add(1, "day")
        .valueOf();
